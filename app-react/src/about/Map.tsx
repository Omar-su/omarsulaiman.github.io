...
const MapCon: React.FC = () => {
  ...
  routePolyline.setMap(map);
    ...
    const newMarker = new window.google.maps.Marker({
      position: { lat: 37.916847, lng: 32.685360 },
      map: map,
      title: 'My home town',
    ...
    function animateRoute(newMarker: { setPosition: (arg0: { lat: number; lng: number; }) => void; }) {
      let count = 0;
      const interval = setInterval(() => {
        if (count === routeCoordinates.length) {
          clearInterval(interval);
          return;
        }
  
        // Move the marker
        newMarker.setPosition(routeCoordinates[count]);
  
        // Pan the map
        map.panTo(routeCoordinates[count]);
